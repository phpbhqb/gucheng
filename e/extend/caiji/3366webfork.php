...
while($classres=$empire->fetch($classquery)){
	...
	$tagclasssql="select classid from {$dbtbpre}enewstagsclass where classname='{$classres['classname']}' limit 1";
	$tagclassquery=$empire->query($tagclasssql);
	$tagclassres=$empire->fetch($tagclassquery);
	$tagclassid=$tagclassres['classid'];
	if(empty($tagclassid)){
		$inserttagsql="insert into {$dbtbpre}enewstagsclass(classname) values('{$classres['classname']}')";
		$empire->query($inserttagsql);
		$tagclassid=$empire->lastid();
	}
