...
while($classres=$empire->fetch($classquery)){
	...
	for($i=1;$i<=$pagenum;$i++){
		...
		$listurl=preg_replace('/_\d+\./','_'.$i.'.',$pageoneurl);
		...
		for($j=$urlnum-1;$j>=0;$j--){
			...
			if(!empty($ad) || !strstr($playflashurl,'.swf')){
				file_put_contents($logpath.'/./'.$classid.'.error.log',$playurl.'有广告或者不为swf'."\t".date( 'Y-m-d H:i:s')."\n",FILE_APPEND);
				continue;
			}
			...
			if(!empty($titlepicurl)){
				$titlepicname=uniqid().getUrlEnd($titlepicurl);
			...
			if(!empty($coverpicurl)){
				$coverpicname=uniqid().getUrlEnd($coverpicurl);
			...
			if(!empty($playflashurl)){
				$playflashname=uniqid().getUrlEnd($playflashurl);
